<template>
  <div id='balanceBuy'>
    ...
    <div class="buy">
        <div class="box1">
            <div class="item" :class="{active: active==index}" @click="activeNav(index)" v-for="(item,index) in itemnav" :key="index">{{item}}</div>
        </div>
        <div class="box2">
          <p><span>请选择金额</span><span>{{card}}</span></p>
          <div class="box1">
            <div class="money" :class="{active2: active2==index}" @click="activeMoney(index,item)" v-for="(item,index) in money" :key="index">{{item}}</div>
          </div>
        </div>
...
export default {
  ...
  created () {
    ...
    if (this.status === '1') {
      //挂卖
      this.name = '提现';
      this.itemnav = ['创建订单','未匹配订单','已匹配订单','已完成订单','卖出中心']
    } else if (this.status === '2') {
      //挂买
      this.name = '购买';
      this.itemnav = ['创建订单','未匹配订单','已匹配订单','已完成订单','买入中心']
    }
  },
  ...
  methods: {
    loading () {
      this.axios.post('transaction/index',{
        token: this.token()
      })
        .then(({data}) => {
          console.log(data)
    ...
    balanceBuy(){
      ...
      if (this.status === '1') {
      //挂卖
      this.axios.post('transaction/dotran',{
        token: this.token(),
        type: '1',
